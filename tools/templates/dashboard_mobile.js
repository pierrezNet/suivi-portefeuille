(function () {
    ...
    const items = data.agenda
      .map((it) => {
        ...
        const ticker = it.ticker
          ? ` <code>${escapeHtml(it.ticker)}</code>`
          : "";
        const notes = it.notes
          ? `<small>${escapeHtml(it.notes)}</small>`
          : "";
        return `
          <li>
            <div class="date">
  ...
  function renderWatchlist(data) {
    if (!data.watchlist_haute || data.watchlist_haute.length === 0) {
      return "";
    }
    const items = data.watchlist_haute
      .map((w) => {
        const ticker = w.ticker
          ? `<code>${escapeHtml(w.ticker)}</code> `
          : "";
        const these = w.these_lt
          ? `<small>${escapeHtml(w.these_lt)}</small>`
          : "";
  ...
  // --- Init ------------------------------------------------------------------
  function init() {
    const tag = document.getElementById("dashboard-data");
    if (!tag) {
    ...
    try {
      ...
      document.getElementById("app").innerHTML =
        "<p>Données invalides : " + escapeHtml(e.message) + "</p>";
      return;
    }
